...
void RobotGeometry::calculateGrad()
{
  ...
  high = (asin((0.5 * rside) / 120.0)) * 2.0; //120mm shank length + Neglegt the offset because not needed
  ...
  if (zmm > 0)
  {
    ...
    low = (PI / 2.0) - acos(rrot / rside) - ((PI - high) / 2.0); //instead of asin acos is more correct. But that was corrected by the angle +Neglegt the offset because not needed
  }
  else
  {
    //low = PI / 2 + acos(rrot / rside) + ((PI - high) / 2.0) - (PI / 2.0);
    low = (PI / 2.0) + acos(rrot / rside) - ((PI - high) / 2.0); //Neglegt the offset because not needed
  }
  //correct higher Angle as it is mechanically bounded width lower Motor
  high = (PI / 2.0) + low - high;
